      ...
      <div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
          {topAnimes.slice(0, 10).map((item, index) => (
            <AnimeCard key={item.mal_id} src={item.images.jpg.image_url} alt={title} index={index} title={item.title} />
          ))}
        </div>
      </div>
      {/* Popular Characters */}
            ...
            >
              <div className="flex flex-col justify-center items-center">
                <div className="overflow-hidden aspect-3/4">
                  <img src={item.images.jpg.image_url} alt={item.name} className="rounded-sm h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
                </div>
