...
import { searchContent } from '../../apis/apiService';
import { useRouteLoaderData } from 'react-router-dom';
import ContentCard from './ContentCard';

const [searchValue, setSearchValue] = useState('');

const handleSearchCinema = async (q) => {
  const query = q.target.value;
  setSearchValue(query);

  if (query.length > 3) {
    const result = await searchContent(searchValue);
...
export const Search = () => {
  return (
    <div className="flex items-center gap-2 border rounded-lg px-2 sm:w-[300px] md:w-[400px] sm:h-11">
      ...
      <input type="text" value={searchValue} onChange={handleSearchCinema} placeholder="Search..." className="rounded-lg p-2  outline-none text-xs sm:text-base lg:text-lg" />
    </div>
  );
};
